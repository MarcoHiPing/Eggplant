...
public class GameManager : MonoBehaviourPunCallbacks
{
    ...
    [Header("Player Settings"), Tooltip("The prefab to use for representing the player")]
    public GameObject blindPlayerPrefab;
    public GameObject deafPlayerPrefab;
    ...
    //    if (ThirdPersonCharacter.LocalPlayerInstance == null)
    //    {
    //        Debug.LogFormat("We are Instantiating LocalPlayer from {0}", SceneManagerHelper.ActiveSceneName);

    //        if(PhotonNetwork.NickName.ToLower().Trim().Contains("blind"))
    //            localPlayer = PhotonNetwork.Instantiate(blindPlayerPrefab.name, startSpawnPoint.position, Quaternion.identity, 0);
    //        else localPlayer = PhotonNetwork.Instantiate(deafPlayerPrefab.name, startSpawnPoint.position, Quaternion.identity, 0);
    //    }
    //    else
    //    {
    ...
    {
        if (localPlayer.GetComponent<ThirdPersonUserControl>() != null)
        {
            var playerControls = localPlayer.GetComponent<ThirdPersonUserControl>();
